      ...
      <CardContent className='space-y-4'>
        {formData.items.map((item, index) => (
          ...
          >
              ...
              {resolveApiAssetUrl(item.documentFile) ? (
                <div className='space-y-2 rounded-md border p-3'>
                  <p className='flex items-center gap-2 text-sm font-medium'>
                    ...
                    {getFileNameFromUrl(item.documentFile) || 'Selected file'}
                  </p>
            ...
            <div className='space-y-2'>
              <Label htmlFor={`annual-report-year-${index}`}>Year</Label>
              <Input
                id={`annual-report-year-${index}`}
                type='text'
                inputMode='numeric'
                value={item.year}
                onChange={(event) => updateYear(index, event.target.value)}
                placeholder='2026'
              />
            </div>
                ...
                >
                  <div className='flex items-center justify-between gap-2'>
                    ...
                    <Button
                      ...
                      size='icon'
                      className='text-destructive hover:bg-destructive/10 hover:text-destructive h-7 w-7'
                      onClick={() => removeKeyPoint(index, keyPointIndex)}
                    >
                      <X className='size-4' />
                    </Button>
                  </div>
                  <Input
                    ...
                    onChange={(event) =>
                      updateKeyPoint(index, keyPointIndex, event.target.value)
                    }
                    placeholder={
                      isKhmer
